...
{
     QList<QVariant>::const_iterator iter;
    for (  iter = values.constBegin(); iter != values.constEnd(); ++iter)
    {
...
QList<QVariant> VariantData::VariantDataPrivate::toListOfVariants(const QStringList &values) const
{
    QList<QVariant> list;
